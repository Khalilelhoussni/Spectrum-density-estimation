fista_nonconvex2 <- function(X, y, lambda, intercept=T, tol = 1e-8,nu=0.1) {
  startTime <- Sys.time()
  n <- nrow(X)
  p <- ncol(X)
  ...
  print("done")
  cost=lasso_cost_NC(X, y, beta, lambda, intercept=intercept,nu=nu)
  ...
  while(continue){
    ...
    beta[i0+(1:p)] <- thresholdfct(beta_temp, lambda/L,nu)
    ...
    cost=c(cost,lasso_cost_NC(X, y, beta, lambda, intercept=intercept, nu=nu))
